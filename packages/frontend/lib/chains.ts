...
export const CHAIN_BY_ID: Record<number, Chain> = Object.fromEntries(
  Object.values(CHAIN_BY_NAME).map((chain) => [chain.id, chain])
);

/**
 * Array of all supported chains for RainbowKit/wagmi
 */
export const SUPPORTED_CHAINS: Chain[] = Object.values(CHAIN_BY_NAME);
...
export function getDefaultChain(): Chain {
  const networkName = process.env.NEXT_PUBLIC_X402_CHAIN || "sepolia";
  return CHAIN_BY_NAME[networkName] || sepolia;
}
