...
export function getCurrencyDisplay(): string {
  ...
  if (currency === "ETH" || currency === "MNT" || currency === "CRO") {
...
export function getChainId(): number {
  const network = getNetwork();
  return CHAIN_IDS[network] || 11155111; // Default to Sepolia
}
...
export function isTestnet(): boolean {
  const network = getNetwork();
  return network.includes("sepolia") || network.includes("testnet") || network.includes("amoy");
}
...
export function getExplorerUrl(): string {
  const network = getNetwork();
  return EXPLORER_URLS[network] || "https://sepolia.etherscan.io";
}
...
export function getUsdcAddress(): `0x${string}` {
  const network = getNetwork();
  return USDC_ADDRESSES[network] || USDC_ADDRESSES["sepolia"];
}
