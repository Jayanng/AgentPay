 ...
 *
 * CAW Mode: When Cobo Agentic Wallet is configured, all x402 payments
 * are routed through CAW's API with automatic policy evaluation.
 * Every payment demonstrates CAW's critical role as the fund gatekeeper.
 */
...
import { wallet, log, getExplorerUrl, getWalletBalance, sendToken, WALLET_MODE } from "../wallet.js";
import { makePayment } from "../payment.js";
import { isCawActive, cawX402Request, getCawAddress } from "../caw-bridge.js";

// ═══════════════════════════════════════════════════════════════════════════
// TOOL DEFINITIONS
// ═══════════════════════════════════════════════════════════════════════════

const walletModeLabel = WALLET_MODE === "caw" ? "via Cobo Agentic Wallet" : "via private key";

export const TOOLS = [
  {
    name: "x402_request",
    description:
      `ACCESS ANY PAID RESOURCE - This tool has a built-in wallet and AUTOMATICALLY PAYS for resources. When the server returns HTTP 402 Payment Required, this tool pays ${CURRENCY} from the configured wallet on ${NETWORK} ${walletModeLabel} and retries the request — no user action needed. YOU MUST USE THIS TOOL to access any resource that costs money. NEVER tell the user you cannot pay — this tool handles payments automatically. Use the 'url' field from x402_list_resources which contains the correct server URL (${SERVER_URL}).${WALLET_MODE === "caw" ? " Payments are routed through Cobo Agentic Wallet with automatic permission policy evaluation." : ""}`,
    inputSchema: {
      type: "object",
      properties: {
        url: {
  ...
  },
  {
    name: "x402_wallet",
    description:
      `Check the agent wallet's ETH and ${CURRENCY} balance. Also shows the wallet address and network.${WALLET_MODE === "caw" ? " This wallet is powered by Cobo Agentic Wallet." : ""}`,
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  },
  {
    name: "x402_send",
    description:
      `Send ${CURRENCY} directly to any wallet address ${walletModeLabel}. For peer-to-peer payments.${WALLET_MODE === "caw" ? " Subject to CAW permission policies." : ""}`,
    inputSchema: {
      type: "object",
      properties: {
        to: {
...
async function x402Request({ url, method = "GET", headers = {}, body, maxPayment, preview = false }) {
  ...
  // CAW Mode: Route through CAW API
  if (isCawActive()) {
    log(`[CAW] x402 request: ${method} ${url}`);

    const result = await cawX402Request({
      url,
      method,
      headers,
      body,
    ...
    if (result) {
      // Add wallet address for prior payment check
      const cawAddr = getCawAddress();
      if (cawAddr && result.status === undefined) {
        // If CAW didn't handle it, fall through to viem mode
        log(`[CAW] Falling back to viem mode for this request`);
      } else {
        return result;
      }
    }
