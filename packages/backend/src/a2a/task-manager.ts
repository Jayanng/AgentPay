...
export async function processPayment(
  ...
  try {
    ...
    if (!rpcUrl && isValidNetwork(netName)) {
      rpcUrl = getChainMetadata(netName as NetworkId).rpcUrl;
    }
    if (!rpcUrl) {
      throw new Error(`No RPC URL for network ${netName}`);
    }
    ...
    const receipt = toAP2Receipt(
      ...
      net.network || payload.network || "sepolia",
      net.chainId,
...
export async function createCartFromIntent(
  ...
  if (items.length === 0) {
    ...
    const emptyCartMandate: CartMandate = {
      contents: {
        ...
        merchant_name: process.env.STORE_NAME || "AgentPay",
      },
    };
  ...
  const cartMandate = buildCartMandate(
    cartId,
    process.env.STORE_NAME || "AgentPay",
    paymentRequest,
    expiresAt,
 ...
 */
async function resolveResourcePrice(
  resourceId: string
): Promise<number> {
  try {
    const { Resource } = await import("../models/index.js");
    const resource = await Resource.findOne({
      $or: [{ _id: resourceId }, { slug: resourceId }],
    });
    if (resource?.priceUsdc) {
      return resource.priceUsdc;
    }
  } catch {
    ...
    if (intent.skus && intent.skus.length > 0) {
      ...
      if (resources.length > 0) {
        return resources.map((r: any) => ({
          label: r.name || `Resource ${r._id}`,
          priceUsd: r.priceUsdc || 1.0,
        }));
      }
    }
