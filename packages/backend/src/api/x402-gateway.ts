...
export async function handleResourceAccess(req: Request, res: Response) {
  ...
  try {
    ...
    // Verify with retries — fast chains may need a moment for RPC sync
    let verified = false;
    const maxRetries = 3;
...
async function serveResource(resource: any, req: Request, res: Response) {
  switch (resource.type) {
    ...
    case "agent":
      return serveAgentResource(resource, req, res);
    default:
...
/**
 * Serve agent service resource
 */
async function serveAgentResource(resource: any, _req: Request, res: Response) {
  const { agent_endpoint } = resource.config || {};
  ...
  if (!agent_endpoint) {
    return res.json({
      ...
      resourceId: resource._id.toString(),
      name: resource.name,
      description: resource.description || "Agent service endpoint",
      message: "Agent endpoint will be configured by the resource owner",
    });
  }
