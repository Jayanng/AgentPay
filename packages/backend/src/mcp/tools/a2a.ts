...
const sendA2AMessageTool = defineTool({
  ...
  description:
    "Send an A2A message to initiate a resource purchase or access task. Returns a task with payment requirements.",
  inputSchema: z.object({
    action: z
      .enum(["purchase", "access-resource"])
      .describe("The action type: purchase or access-resource"),
    resourceId: z
      .string()
      .describe("Resource ID or slug (required for both purchase and access-resource)"),
  }),
  handler: async ({ action, resourceId }) => {
    if (action === "purchase" || action === "access-resource") {
