...
describe("CHAINS constant", () => {
  it("should have all 13 networks", () => {
    expect(Object.keys(CHAINS).length).toBe(13);
  });
...
describe("TOKEN_ADDRESSES", () => {
  ...
  it("should have Sepolia USDC address", () => {
    expect(TOKEN_ADDRESSES["sepolia"].USDC).toBe(
...
describe("getRpcEndpoint", () => {
  ...
  it("should return default public RPCs", () => {
    ...
    expect(getRpcEndpoint("sepolia")).toContain("sepolia");
  });
});
...
describe("createPaymentTransaction", () => {
  ...
  it("should create ETH transaction for native ETH (Sepolia)", () => {
    const tx = createPaymentTransaction({
      scheme: "exact",
      network: "sepolia",
      chainId: 11155111,
