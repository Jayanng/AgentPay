 ...
 */
export const TOKEN_ADDRESSES: Record<Network, Record<Exclude<TokenType, "ETH" | "CRO" | "MNT" >, Address>> = {
  mainnet: {
  ...
  "base-sepolia": {
    USDC: "0xa059e27967e5a573a14a62c706ebd1be75333f9a", // mUSDC on Base Sepolia
  ...
  "mantle-sepolia": {
    USDC: "0x036CbD53842c5426634e7929541eC2318f3dCF7e" as Address, // Circle USDC on Mantle Sepolia
    USDT: "0x0000000000000000000000000000000000000000" as Address,
    DAI: "0x0000000000000000000000000000000000000000" as Address,
  },
  cronos: {
...
export function createPaymentTransaction(
  ...
  // Native tokens (ETH on Ethereum, CRO on Cronos, MNT on Mantle, , etc.)
  if (requirements.token === "ETH" || requirements.token === "CRO" || requirements.token === "MNT") {
    return createETHPaymentTransaction(recipient, amount);
  } else {
    const tokenAddress = TOKEN_ADDRESSES[requirements.network][requirements.token as Exclude<TokenType, "ETH" | "CRO" | "MNT">];
...
export async function verifyPaymentTransaction(
  ...
  try {
    ...
    // the transaction was included in a block. On fast chains or due to
    ...
    if (requirements.token === "ETH" || requirements.token === "MNT" || requirements.token === "CRO") {
